...
Compiling a patched kernel is also recommended to some users in the forums, however, its just a chore to do for every kernel version, especially if you have a low-end hardware.
...
You should have create and add your MOK to the shim :
...
Once this is done, you will need to early-load this module and add `lockdown_hibernate.enable=1` to the kernel command-line. (You can search for how to do both on your distro's documentation). 
...
Reboot. You can now test it with `sudo systemctl hibernate` :-)
